module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Kirito",
  description: "Random photo of ayaka",
...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 0) api.sendMessage("You need $100 to see the photo",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 0})
   var callback = () => api.sendMessage({body:`Here's a Ayaka photo for you 💞 ${link.length}\n-100 usd!`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
