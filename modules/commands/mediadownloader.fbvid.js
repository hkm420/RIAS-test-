...
module.exports.run = async function ({ api, event, args }) {
  if (args.length === 0) {
    await api.sendMessage('Please provide a valid Facebook video URL.', event.threadID,event.messageID);
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      await api.sendMessage({ attachment: videoStream }, event.threadID, event.messageID);
