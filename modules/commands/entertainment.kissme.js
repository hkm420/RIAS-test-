...
module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "Kirito",
    description: "Kiss someone by tagging him",
  usePrefix: true,
...
module.exports.run = async function ({ event, api, args, Currencies }) { 
    ...
    var two = senderID, one = mention[0];
  await Currencies.increaseMoney(event.senderID, parseInt(hc*rd));
  
  if (!two) return api.sendMessage("Tag 1 person Bitch", threadID, messageID);
  else {
        return makeImage({ one, two }).then(path => api.sendMessage({ body: `Babe Your lips, my lips, apocalypse✨`, attachment: fs.createReadStream(path)}, threadID, () => fs.unlinkSync(path), messageID));
  }
}