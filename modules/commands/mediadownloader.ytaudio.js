...
const handleReply = async ({ api, event, handleReply }) => {
     try {
         ...
         if (fs.statSync(data).size > 540907080) return api.sendMessage('⚠️The file could not be sent because it is larger than 50MB.', event.threadID, () => fs.unlinkSync(path), event.messageID);
...
const run = async function ({ api, event, args }) {
     ...
     if (args[0]?.startsWith("https://")) {
         try {
             ...
             if (fs.statSync(data).size > 540907080) {
                 return api.sendMessage('⚠️The file could not be sent because it is larger than 50MB.', event.threadID, () => fs.unlinkSync(data), event.messageID);
             }
