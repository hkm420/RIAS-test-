module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "Kirito",
    description: "Make a couple selfie by tagging her",
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Tag someone Bitch", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Say cheeseee",
            mentions: [{
          tag: tag,
          id: mention
        }],
