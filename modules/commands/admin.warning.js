        ...
        default: {
            if (permssion != 2) {
                ...
                try {
                    ...
                    if (event.messageReply.senderID == api.getCurrentUserID()) return api.sendMessage('Unable to alert bot account.', threadID, messageID);
                    ...
                    return api.sendMessage(`Warned ${name} with reason: ${args.join(" ")}, ${(data.banned) ? `Because I got warned 3 times, the above account has been banned` : `The above account also ${data.warningLeft} warning turn!`}`, threadID, messageID);
