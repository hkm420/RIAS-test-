...
module.exports.run = async ({ api, event }) => {
  ...
  try {
    ...
    stream.on('end', () => {
      ...
      if (fs.statSync(filePath).size > 5409070800) {
        fs.unlinkSync(filePath);
        return api.sendMessage('⚠ | ERROR The file could not be sent because it is larger than 50MB.', event.threadID);
      }

      const message = {
        body: `🎵 | Here's your music, provided by: ${global.config.BOTNAME}\n\nTitle: ${title}\nArtist: ${artist}`,
